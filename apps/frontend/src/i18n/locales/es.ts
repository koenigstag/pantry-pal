...
export const es = {
  ...
  fieldErrors: {
    ...
    fillPercent: 'Elige cuánto queda.',
  },
  ...
  subItems: {
    ...
    unopened: 'Sin abrir',
    opened: (date: string) => `Abierto el ${date}`,
    left: (percent: string) => `Queda un ${percent}`,
    goodUntil: (date: string) => `Usar antes del ${date}`,
    addMore: 'Añadir más',
    openToday: (count: number) => (count === 1 ? 'Abrirlo hoy' : 'Abrir uno hoy'),
    howMuchLeft: 'Cuánto queda',
    opensToday: (full: string) => `Por debajo del ${full}, cuenta como abierto hoy.`,
    changeDates: 'Cambiar fechas',
    usedUp: (count: number) => (count === 1 ? 'Lo he usado' : 'He usado uno'),
    threwOut: (count: number) => (count === 1 ? 'Lo he tirado' : 'He tirado uno'),
    deleteOne: (count: number) => (count === 1 ? 'Eliminarlo' : 'Eliminar uno'),
    deleteHint: 'Añadido por error',
    addTitle: 'Añadir más',
    add: 'Añadir',
    changeTitle: 'Cambiar fechas',
    applyTo: 'Aplicar a',
    outOf: (total: number) => `Cuántos de los ${formatNumber(total)} de esta fila`,
  },
